...
const { encodeDepositRequest, decodeDepositResponse } = require('../../protocols/codecs.js');
...
module.exports = async function deposit({ socket, clientId, requestId },
    ...
    const bodyBuffer = encodeDepositRequest({ name, password, accountNo, currency, amount });
    const packet = buildPacket(OP_CODE.DEPOSIT, clientId, requestId, bodyBuffer);

    try {
        const encodedReply = await socketSend(socket, packet);
        const reply = decodeDepositResponse(encodedReply);
        console.log("\nResponse:", reply);
    } catch (err) {
